...
class mapThat:
    ...
    def get_default_mode(self):
        mode_flag=int(input("1. Select a default mode of transport\n2.Select mode of transport for each event\n"))
        if mode_flag==1:
    ...
    def get_default_time_bw_events(self):
        self.time_bw_event=int(input("Enter the max time in mins between 2 events to go directly from one event to another:\n"))*60
        if os.path.exists(self.user_data):
            with open(self.user_data) as infile:
                self.data=json.load(infile)
    ...
    def check_user_Data(self):
        if  not os.path.exists(self.user_data):
            ...
            with open(self.user_data) as json_file:
                    ...
                    print("Max time in mins between 2 events to go directly from one event to another:",str(self.time_bw_event))
        ...
        for event in self.events:
            ...
            if 'description' in event:
                if event['description']=='#Created by MapThat#':
                    ...
                    print("travel event")
                    continue
                if '#This event has been checked by MapThat#' in event['description'] and self.prev_event_traversed==1:
                    self.prev_time=datetime.datetime.strptime((event['end'].get('dateTime', event['end'].get('date'))), "%Y-%m-%dT%H:%M:%S%z")
            ...
            if self.prev_event_travel==1 and self.prev_travel_event_id not in [None]:
                ...
                self.service.events().delete(calendarId='primary', eventId=self.prev_travel_event_id).execute()
